...
/*-----------------------------------------------------------------------------------*/
/*	Posts Widget Class
/*  Borrowed and modified from Shamrock https://wordpress.org/themes/flipster/ License: GPL V2
/*-----------------------------------------------------------------------------------*/

class Flipster_Posts_Widget extends WP_Widget {

	var $defaults;

	function __construct() {
		$widget_ops = array( 'classname' => 'flipster_posts_widget', 'description' => __( 'Display your posts with this widget', 'flipster' ) );
		$control_ops = array( 'id_base' => 'flipster_posts_widget' );
		parent::__construct( 'flipster_posts_widget', __( 'Flipster Posts', 'flipster' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => __( 'Featured Posts', 'flipster' ),
			'numposts' => 5,
			'category' => array(),
			'orderby' => 0,
			'date_limit' => 0,
	...
	function widget( $args, $instance ) {
		...
		$flipster_posts = new WP_Query( $q_args );

		if ( $flipster_posts->have_posts() ): ?>
			<ul class="tm-posts-widget uk-list uk-list-space">

				<?php while ( $flipster_posts->have_posts() ) : $flipster_posts->the_post(); ?>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'flipster' ); ?>:</label>
		...
		<p>
	   	 	<label for="<?php echo $this->get_field_id( 'numposts' ); ?>"><?php _e( 'Number of posts to show', 'flipster' ); ?>:</label>
	  ...
	  <p>
		...
		<label for="<?php echo $this->get_field_id( 'auto_detect' ); ?>"><?php _e( 'Auto detect category', 'flipster' ); ?></label>
		<small class="howto"><?php _e( 'If sidebar is used on single post template, display posts from current post category ', 'flipster' ); ?></small>
	  </p>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'date_limit' ); ?>"><?php _e( 'Only select posts which are not older than', 'flipster' ); ?>:</label>
	...
	<p>
	   	 <label for="<?php echo $this->get_field_id( 'manual' ); ?>"><?php _e( 'Choose manually', 'flipster' ); ?>:</label>
		 ...
		 <small class="howto"><?php _e( 'Specify post ids separated by comma if you want to select only those posts. i.e. 213,32,12,45 Note: you can also choose pages as well as custom post types', 'flipster' ); ?></small>
	</p>
	...
	function dates_q() {
		$dates = array(
			'-1 day' => __( '1 Day', 'flipster' ),
			'-1 week' => __( '1 Week', 'flipster' ),
			'-1 month' => __( '1 Month', 'flipster' ),
			'-3 months' => __( '3 Months', 'flipster' ),
			'-6 months' => __( '6 Months', 'flipster' ),
			'-1 year' => __( '1 Year', 'flipster' ),
			'0' => __( 'Select all posts', 'flipster' )
		);
	...
	function widget_orderby( $widget_instance = false, $orderby = false ) {
		...
		$orders['date'] = __( 'Published date', 'flipster' );
		$orders['comment_count'] = __( 'Number of comments', 'flipster' );
		$orders['rand'] = __( 'Random', 'flipster' );

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo $widget_instance->get_field_id( 'orderby' ); ?>"><?php _e( 'Order by:', 'flipster' ); ?></label>
...
?>
				<label for="<?php echo $widget_instance->get_field_id( 'category' ); ?>"><?php _e( 'Choose from:', 'flipster' ); ?></label><br/>
	...
	function widget_meta( $widget_instance = false, $current = false ) {
		...
		$metas = array(
			'0' => __( 'None', 'flipster' ),
			'date' => __( 'Date/time', 'flipster' ),
			'author' => __( 'Author', 'flipster' ),
			'comments' => __( 'Comments', 'flipster' ),
			'rtime' => __( 'Reading time', 'flipster' )
		);

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo $widget_instance->get_field_id( 'meta' ); ?>"><?php _e( 'Display meta data:', 'flipster' ); ?></label>
